...
class MVDParserAsync:
    ...
    async def get_csrf_token(self, html_str) -> str:
        """
    ...
    async def fetch_get_request(self, url: str, headers) -> httpx.Response:
        """
    ...
    async def fetch_post_request(self, url: str, params, headers) -> httpx.Response:
        """
    ...
    async def initialize(self) -> str:
        ...
        """
        response = await self.fetch_get_request(BASE_URL, HEADERS["h1"])
        csrf_token = await self.get_csrf_token(response.text)
        self.params["sec_csrftoken"] = csrf_token

        response = await self.fetch_get_request(CAPTCHA_URL, HEADERS["h2"])
    ...
    async def get_result(self, captcha_word: str) -> dict:
        ...
        response = await self.fetch_post_request(BASE_URL, self.params, HEADERS["h3"])
